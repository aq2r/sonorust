...
pub async fn wav(
    user_id: UserId,
    text: &str,
) -> Result<(Option<CreateAttachment>, Option<&str>), SonorustError> {
    ...
    let Ok(voice_data) = Sbv2Client::infer_from_user(text, &userdata).await else {
        ...
        return Ok((None, Some(lang_t!("wav.fail_infer", lang))));
    };

    Ok((Some(CreateAttachment::bytes(voice_data, "audio.mp3")), None))
}
