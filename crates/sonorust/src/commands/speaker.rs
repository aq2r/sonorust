...
pub async fn speaker(
    ...
    // 話者の数が 26 以上ならページ移動ボタンを追加
    if model.id2spk.len() >= 26 {
...
fn create_embed(model: &ModelInfo) -> CreateEmbed {
    ...
    let title = format_t!("speaker.embed.title", lang, model.model_name);
...
fn create_select_menu(model: &ModelInfo) -> CreateSelectMenu {
    ...
    for i in 0..=24 {
        match model.id2spk.get(&i) {
            Some(spk_name) => selectoption_vec.push(CreateSelectMenuOption::new(
                spk_name.as_str(),
                format!("{}||{}", model.model_name, spk_name),
            )),
            None => break,
        }
    }
